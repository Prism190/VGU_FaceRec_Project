...
\begin{table}[h]
\centering
\caption{Occlusion augmentation parameters per training variant. V2 uses aggressive blur; V3 uses milder blur. ``—'' = not used.}
...
\hline
Clean epochs (no aug) & 20 & 10 & 10 \\
Mask prob (ramp start$\to$end) & 0.10 (static) & 0.10$\to$0.30 & 0.10$\to$0.30 \\
Mask ramp epochs & ep.~20--40 & ep.~10--25 & ep.~10--25 \\
Gaussian blur prob & 0 & 0.25$\to$0.75 & 0.25$\to$0.75 \\
Gaussian $\sigma$ & — & 1.0$\to$2.8 & 1.0$\to$1.5 \\
Gaussian kernel size (odd) & — & 5--11 & 5--11 \\
Motion blur prob & 0 & 0.10$\to$0.55 & 0.10$\to$0.55 \\
Motion kernel size (odd) & — & 7--17 & 5--9 \\
\hline
\end{tabular}
}
\end{table}
...
We compare six checkpoints. All variants use the same concept of "latest" (final epoch) and "best" (epoch with highest mean TAR@$10^{-4}$ on validation bin protocols):
\begin{itemize}
