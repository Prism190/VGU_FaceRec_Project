...
Table~\ref{tab:occlusion_results} reports the drop in TAR under synthetic lower-face masking. A smaller absolute drop indicates better robustness. V3/SWA consistently reduces the degradation caused by occlusion across all evaluated protocols.

\begin{table}[h]
\centering
\caption{Occlusion robustness measured by TAR drop under lower-face masking. Smaller absolute drop is better.}
...
Compared with V1/latest, V3/SWA reduces the TAR drop on all occlusion benchmarks. The improvement is especially clear on AgeDB, CPLFW, and CALFW. This supports the use of V3/SWA in masked or partially occluded environments, even though it does not achieve the best clean verification accuracy\cite{cplfw,cfp,agedb,izmailov2019averagingweightsleadswider}.

The teacher also suffers large degradation under lower-face masking, especially on CFP-FP, AgeDB, and CPLFW. This shows that a larger model is not necessarily more robust to structured occlusion. The robust student variant benefits from occlusion-aware training and SWA, making it more stable under masked-face conditions.
...
MobileFaceNet (W600K) & $\sim$1M & 89.42 & — \\
Student: V1/latest & 9.58M & \textbf{87.98} & \textbf{90.65} \\
Student: V3/SWA & 9.58M & 85.27 & 87.77 \\
\hline
\end{tabular}
\end{table}

MobileFaceNet (W600K) has 1M parameters and 7$\times$ more training identities, yet our 9.58M-parameter student is within 1.44\% on IJB-B (87.98 vs.~89.42). On IJB-C, the student surpasses MBF in IJB-C evaluation at 90.65\% (MBF IJBC was not measured due to CPU-only ONNX inference requiring $>$4 hours). The student's advantage is its combined clean-face accuracy and occlusion robustness — a regime where MBF, trained only on clean faces, is significantly weaker (see RMFRD evaluation below).
...
\begin{table}[h]
\centering
\caption{Real-world masked face recognition on RMFRD (403 paired identities, RetinaFace-aligned).}
\label{tab:rmfrd}
\begin{tabular}{lcccc}
\hline
...
(Results from the RetinaFace-aligned RMFRD evaluation are pending; table will be updated. Reference unaligned results: MBF 68.72\% AUC / 7.25\% Rank-1; V1 71.02\% / 4.78\%; V3/SWA 69.90\% / 6.84\%. Aligned numbers are expected to be substantially higher and more meaningful, as simple resize from unaligned crops suppresses all models uniformly.)
