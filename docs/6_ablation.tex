...
In contrast, V3/SWA is not the strongest model under clean verification, but it is the most robust under lower-face masking. This suggests that robustness-oriented training can reduce sensitivity to occluded facial regions, but may slightly sacrifice clean benchmark performance. Therefore, the best checkpoint should be selected according to the target deployment condition: V1 for clean access-control scenarios and V3/SWA for masked or partially occluded environments\cite{izmailov2019averagingweightsleadswider}.
...
\begin{table*}[h]
\centering
\caption{Template pooling ablation on IJB-B/C. TAR at FAR=$10^{-4}$ is the primary metric for access control. All rows use the same checkpoint per model (V1/best = \texttt{phase1\_best.pt}; V3/SWA = SWA-averaged checkpoint). ``—'' = pending.}
...
V1/best & magface\_weighted & 99.38 & 93.01 & \textbf{86.88} & 99.54 & 94.51 & \textbf{89.50} \\
V1/best & top5             & — & — & — & — & — & — \\
V1/best & top10            & — & — & — & — & — & — \\
V3/SWA  & mean             & 99.20 & 92.23 & 85.11 & 99.32 & 93.51 & 87.74 \\
V3/SWA  & magface\_weighted & 99.19 & 92.23 & \textbf{85.27} & — & — & — \\
V3/SWA  & top5             & — & — & — & — & — & — \\
V3/SWA  & top10            & — & — & — & — & — & — \\
\hline
\end{tabular}
...
Mean pooling produces slightly higher AUC overall but underperforms \texttt{magface\_weighted} at strict FAR ($10^{-4}$), the operating point relevant for access control. For V1/best on IJB-B, \texttt{magface\_weighted} gains 0.27\% over mean (86.88\% vs.\ 86.61\%) at TAR@$10^{-4}$; on IJB-C the gain is 0.37\% (89.50\% vs.\ 89.13\%). For V3/SWA on IJB-B, the gain is 0.16\% (85.27\% vs.\ 85.11\%). This validates the design choice: quality-weighting by embedding norm consistently prioritizes higher-quality face observations and achieves better calibration at low FAR. (V3/SWA IJBC magface\_weighted and top5/top10 results are pending from the running ablation. The V1/latest checkpoint achieves 87.98\% IJBB TAR@$10^{-4}$ with \texttt{magface\_weighted}, reported in Table~\ref{tab:ijb_results}.)
